...
class MyViewOneViewController: UIViewController {
    ...
    @IBAction func actionButtonNavigation(_ sender: Any) {
        if let myViewOneNavigation = UIStoryboard(name: "MyViewOneNavigation", bundle: nil).instantiateInitialViewController() as? MyViewOneNavigarionViewController {
            navigationController?.pushViewController(myViewOneNavigation, animated: true)
        }
    }
